...
七个模块已接通；当前 1338 项测试通过，但完整 khengine 扫描和 5–10 秒性能目标尚未达到。不能判断的函数单列原因，不猜成 Getter 或 Setter，不生成相应补丁。
...
普通外部调用按需生成程序集、读取函数；接口／虚函数与反射单名称方法查找已复用方法目标，同轮已证明的 Setter 不重复证明。完整动态候选查询仍可能生成全部源码程序集，调用环境也存在重复展开，尚未实现全流程高性能分析。

已确认的下一主干是：源码直接读取、不生成游戏程序集；每个函数只保留一份修改说明，沿固定调用关系传递，不按调用路径复制环境。当前生产代码尚未切换，不能把该方向称为已实现。

后续每个模块先提出具体改法、与用户讨论明确后实施；由主 Agent 亲自实现、测试和审查，不使用子 Agent。每次主干改动用一次完整 khengine 运行及报告验收，不以局部测试替代，也不边跑边补特例。
